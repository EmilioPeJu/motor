...
Version:	1.7.4.2.2.1
Modified By:	sluiter
Last Modified:	2004/02/10 16:59:25
*/
 ...
 * .05  02-19-97 tmm fixed for EPICS 3.13
	      ...
	      struct board_stat *sptr[])
{
    ...
    struct board_stat *brdptr;
    int card, signal;
    BOOLEAN initEncoder, initPos, initString, initPID;
...
/*jps: setting the encoder ratio was moved from init_record() remove  callbacks during iocInit */
    ...
    initPID = (mr->msta & GAIN_SUPPORT) ? ON : OFF;
    
    /* Program the device if an encoder is present */
    if (initPos == ON || initEncoder == ON || initString == ON || initPID == ON)
    {
