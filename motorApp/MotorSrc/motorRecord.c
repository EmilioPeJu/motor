...
Version:	1.21.4.12.2.4
Modified By:	sluiter
Last Modified:	2004/09/09 20:07:16
*/
 ...
 * .44 09-30-02 rls Bug fix for another "invalid state" scenario (i.e., new
 *			target position while MIP != DONE, see README).
 * .45 10-29-02 rls - NTM field added for Soft Channel device.
 *		    - Update "last" target position in do_work() when stop
 *			command is sent.
 * .46 03-21-03 rls - Elminate three redundant DMOV monitor postings.
 *		    - Consolidate do_work() backlash correction logic.
 * .47 04-16-03 rls - Home velocity field (HVEL) added.
 * .48 06-03-03 rls - Set DBE_LOG on all calls to db_post_events().
 * .49 10-29-03 rls - If move is in the preferred direction and the backlash
 ...
 * .50 10-31-03 rls - Fix for bug introduced with R4.5.1; record locks-up when
 *			BDST != 0, DLY != 0 and new target position before
 *			backlash correction move.
 *		    - Update readback after DLY timeout.
 * .51 12-09-03 rls - Bug fix for tweaks ignored. When TWV < MRES and user
 *			does TWF followed by TWR; then, a single TWF
 *			followed by a single TWR appear to be ignored.
 * .52 12-09-03 rls - Bug fix for motor record lock-up when DC servo moving
 *			away from limit switch. Set CDIR before sending motor
 *			commands.
 * .53 12-17-03 rls - Added status update field (STUP).
 * .54 12-19-03 rls - Prevent STUP from activating DLY or setting DMOV.
 * .55 02-10-03 rls - Update lval in load_pos() if FOFF is set to FROZEN.
 * .56 07-20-04 rls - Active STUP whenever do_work is called without anything
 *                    to do.
 */
...
static long do_work(motorRecord *, long);
...
static long init_record(motorRecord * pmr, int pass)
{
    ...
    db_post_events(pmr, &pmr->dly, DBE_VAL_LOG);
...
*******************************************************************************/
static long do_work(motorRecord * pmr, long proc_ind)
{
    struct motor_dset *pdset = (struct motor_dset *) (pmr->dset);
    int dir_positive = (pmr->dir == motorDIR_Pos);
    int dir = dir_positive ? 1 : -1;
    ...
    */
	...
	else
	{
	    ...
	    if (((use_rel == OFF) && ((pmr->dval > pmr->ldvl) == (pmr->bdst > 0))) ||
		((use_rel == ON)  && ((pmr->diff > 0)         == (pmr->bdst > 0))))
		preferred_dir = ON;
	    else
		preferred_dir = OFF;
		...
		if ((fabs(pmr->bdst) <  fabs(pmr->mres)) ||
		    (preferred_dir == ON && pmr->bvel == pmr->velo &&
		     pmr->bacc == pmr->accl))
		{
		    velocity = vel;
		    accel = acc;
