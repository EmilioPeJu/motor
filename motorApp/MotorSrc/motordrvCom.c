...
Version:	1.7.4.1.4.1
Modified By:	sluiter
Last Modified:	2004/04/26 21:41:13
*/
 ...
 * .23  04/26/04  rls   Bug fix for excessive query_axis() delay.
 *
 */
	       ...
	       int a6, int a7, int a8, int a9, int a10)
{
    ...
    STATUS sem_ret;
    int wait_time, itera, query_delay;
	...
	else if (query_delay != 0)
	{
	    wait_time = query_delay;
	    query_delay = 0;
	}
	else
	{
	...
	if (*tabptr->any_inmotion_ptr)
	{
		...
		if (brdptr != NULL && brdptr->motor_in_motion)
		    query_delay = query_axis(itera, tabptr, tick_used);
	    }
	}
	if (sem_ret != ERROR)
...
static int query_axis(int card, struct driver_table *tabptr, ULONG tick)
{
	...
	if (motor_motion != 0)
	{
		...
		{
		    Debug(5, "query_axis(): query_done call at tick = %ld\n", tick);
...
static void process_messages(struct driver_table *tabptr, ULONG tick)
{
	    ...
	    (axis >= 0 && axis < (*tabptr->card_array)[card]->total_axis))
	{
		...
		{
		    Debug(5, "process_messages(): delay = %d\n", (2-delay));
		    taskDelay((int) (2 - delay));	/* 2 RTOS tick delay. */
		}

		if (tabptr->strtstat != NULL)
		    (*tabptr->strtstat) (card);
		Debug(5, "process_messages(): update status at tick = %ld\n", tick);
