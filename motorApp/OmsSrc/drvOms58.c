...
Version:	1.8.4.7.2.1
Modified By:	sluiter
Last Modified:	2004/02/10 17:08:45
*/
 ...
 * .23  06-04-03 rls    extended device directive support to PREM and POST.
 * .24  12-09-03 rls	- Bug fix for motor record lock-up when DC servo moving
 *			away from limit switch.
 * .25  02-06-04 rls    - Eliminate erroneous "Motor motion timeout ERROR".
 */
...
int send_mess(int card, char const *com, char c);
...
errorexit:	errMessage(-1, "Invalid device directive");
     ...
     * MSTA.RA_DIRECTION.
     */
    if (ls_active == ON && ((motor_info->status & GAIN_SUPPORT) != 0) &&
	((motor_info->status & EA_POSITION) == 0) && nodeptr != 0)
    {
	struct motorRecord *mr = (struct motorRecord *) nodeptr->mrecord;


	if (mr->cdir != (short) (motor_info->status & RA_DIRECTION))
	    mr->cdir = (motor_info->status & RA_DIRECTION);
    }
...
/* send_mess()			     */
/*****************************************************/
int send_mess(int card, char const *com, char inchar)
{
    volatile struct vmex_motor *pmotor;
    epicsInt16 putIndex;
