 ...
 * Modification Log:
 * -----------------
 * .00  11-24-2003      kag     initialized from devMCB4B.c
 */
...
#else
#define Debug(L,FMT,V...) {  if(L <= devMicosDebug) \
...
STATIC long Micos_init_record(struct motorRecord *);
STATIC long Micos_start_trans(struct motorRecord *);
STATIC long Micos_build_trans(motor_cmnd, double *, struct motorRecord *);
STATIC long Micos_end_trans(struct motorRecord *);

struct motor_dset devMicos =
{
    {8, NULL, Micos_init, Micos_init_record, NULL},
    motor_update_values,
    Micos_start_trans,
    Micos_build_trans,
    Micos_end_trans
...
STATIC long Micos_init(int after)
{
    ...
    if (after == 0)
    {
        rtnval = symFindByNameEPICS(sysSymTbl, "_Micos_access",
                    (void *) &drvtabptr, &type);
        if (rtnval != OK)
            return(rtnval);
    /*
        IF before DB initialization.
            Initialize Micos driver (i.e., call init()). See comment in
                drvMicos.c init().
        ENDIF
    */
        (drvtabptr->init)();
...
STATIC long Micos_init_record(struct motorRecord *mr)
{
    long rtnval;
...
STATIC long Micos_end_trans(struct motorRecord *mr)
{
    return(OK);
}

/* add a part to the transaction */
STATIC long Micos_build_trans(motor_cmnd command, double *parms, struct motorRecord *mr)
{
    ...
    rtnval = motor_start_trans_com(mr, Micos_cards);
